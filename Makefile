...
run: setup ## Run in foreground
	docker run -it --name minecraft -p 25565:25565 $(ENV) $(VOL) fl/minecraft:latest

login: ## Login
	docker exec -it minecraft bash

rund: setup ## Run in background
	docker run -d --name minecraft -p 25565:25565 -p 25565:25565 $(ENV) $(VOL) fl/minecraft:latest
